...
# Automatically extract USER2 email from IAM Policy JSON
export USER2=$(gcloud projects get-iam-policy $PROJECT_ID --format="json" | grep -o '"user:[^"]*"' | cut -d'"' -f2 | grep -v "$CURRENT_USER" | head -n 1)
